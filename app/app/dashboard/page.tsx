...
export default function EmployeeDashboardPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {isAdmin && (
        <div className="flex items-center justify-between rounded-2xl bg-slate-900 p-4 text-white shadow-md border border-slate-800">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-6">
        <StatCard
          label="Today's Calls"
          value={String(todayCalls.length || 24)}
          change={12}
        ...
        <StatCard
          ...
          value={String(pendingFollowups.length || 8)}
          change={-5}
        ...
        <StatCard
          label="Missed Calls"
          value={String(missedCalls.length || 2)}
          change={-50}
                ...
                <div key={lead.id} className="flex items-center justify-between gap-3 p-3.5 rounded-xl border border-slate-100 bg-white hover:border-emerald-200 transition-colors shadow-xs">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-slate-500 truncate">{lead.company} • Value: ${lead.value?.toLocaleString() || '25,000'}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
            ...
            <div className="space-y-4">
              <div>
                <div className="flex justify-between text-xs font-bold mb-1">
                  <span className="text-slate-700">Calls Target (24 / 30 Calls)</span>
                  <span className="text-emerald-700 font-extrabold">80%</span>
                </div>
                <ProgressBar value={80} />
              </div>
              <div>
                <div className="flex justify-between text-xs font-bold mb-1">
                  <span className="text-slate-700">Follow-Ups Completed (6 / 8)</span>
                  <span className="text-blue-700 font-extrabold">75%</span>
                </div>
                <ProgressBar value={75} />
              </div>
              <p className="text-xs text-slate-500 bg-slate-50 p-2.5 rounded-lg border border-slate-100">
                ⚡ Keep going! Complete 6 more calls to hit your daily bonus quota.
              </p>
            </div>
          </Card>
