...
export function Preloader({ onComplete }: PreloaderProps) {
  ...
  useEffect(() => {
    ...
    if (hasSessionPreloaded || (typeof window !== 'undefined' && sessionStorage.getItem('vocalyze_preloaded'))) {
      setDone(true);
      onCompleteRef.current?.();
      return;
    }
    ...
    const finish = () => {
      if (finished) return;
      finished = true;
      hasSessionPreloaded = true;
      if (typeof window !== 'undefined') {
        sessionStorage.setItem('vocalyze_preloaded', 'true');
      }
      setDone(true);
      onCompleteRef.current?.();
    };
  ...
  return (
    <div ref={rootRef} className="pl-loading-screen" style={{ pointerEvents: 'none' }}>
      ...
      <style>{`
        ...
        @media (max-width: 640px) {
          .pl-counter { left: 20px; bottom: 20px; font-size: 64px; line-height: 66px; clip-path: polygon(0 0, 100% 0, 100% 64px, 0 64px); }
          .pl-num { height: 64px; line-height: 66px; }
